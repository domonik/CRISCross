#!/bin/bash
#SBATCH --job-name=full_multiseed_loso
...
echo "Running full-finetuning multi-seed LOSO for: $TEST_GUIDE (task $SLURM_ARRAY_TASK_ID)"
...
mkdir -p results/full_multiseed_parts

python examples/full_finetuning_multiseed.py \
    --test_guide "$TEST_GUIDE" \
    --results_file "results/full_multiseed_parts/loso_${TEST_GUIDE}.csv" \
    --accelerator gpu
