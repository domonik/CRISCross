#!/bin/bash
#SBATCH --job-name=full_single_sanity
#SBATCH --partition=gpu-single
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=00:30:00            # single guide, single seed - quick sanity check
#SBATCH --output=logs/slurm/full_single_%j.out
#SBATCH --error=logs/slurm/full_single_%j.err

set -euo pipefail

# Sanity-check run: ONE guide, ONE seed. Run this before launching the full
# multi-guide/multi-seed sweep (run_full_finetuning_multiseed.sh) to catch
# config/data/checkpoint problems quickly and cheaply.
#
# Usage:
#   sbatch run_full_finetuning_single.sh [TEST_GUIDE] [SEED]
#   (defaults: TEST_GUIDE=sg1, SEED=0)

TEST_GUIDE=${1:-sg1}
SEED=${2:-0}

echo "Running full-finetuning sanity check for: $TEST_GUIDE (seed=$SEED)"
...
python examples/full_finetuning_single.py \
    --test_guide "$TEST_GUIDE" \
    --seed "$SEED" \
    --results_file "results/full_sanity_check.csv" \
    --accelerator gpu
