...
if __name__ == "__main__":
    ...
    parser.add_argument("--results_file", type=str, default="results/full_loso_results.csv",
                        help="CSV file to write per-guide per-seed AUPRC results")
    ...
    for seed_idx, seed in enumerate(SEEDS):
        print(f"\n{'='*60}")
        print(f"[{TEST_GUIDE}] Seed {seed} ({seed_idx + 1}/{len(SEEDS)})")
        print(f"{'='*60}")

        pl.seed_everything(seed, workers=True)

        # ============ Load Pretrained Model (all params trainable) ============
        # Reloaded fresh each seed so fine-tuning always starts from the same pretrained weights.
        model, cfg = load_criscross_from_pretrained(args.pretrain_ckpt)
        lightning_model = CRISPRFullWrapper(model, lr=1e-4)

        # ============ Load Data ============
        # Recreated each seed so the random val split is re-seeded.
        # epi_features/window_size must match what the checkpoint was pretrained with.
        ...
        trainer = pl.Trainer(
            default_root_dir=f"checkpoints_full/{TEST_GUIDE}/seed_{seed}",
            max_epochs=200,
        ...
        # ============ Save Result (incremental, deduplicated by guide+seed) ============
        new_row = {
            "guide_id": TEST_GUIDE,
            "seed": seed,
